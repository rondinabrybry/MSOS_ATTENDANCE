<x-app-layout>
    ...
    <button type="button" class="manual-attendance-button">
        <a href="{{ url('/admin/manual') }}" class="btn btn-xs btn-info">Manual Attendance</a>
    </button>
                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr
                                        ...
                                        <td class="w-4 p-4">
                                            <div class="ps-3">
                                                <div class="text-base font-semibold text-black dark:text-white">
                                                    {{ $user->student_id }}</div>
                                                <div class="font-normal text-gray-500">{{ $user->rf_id }}</div>
                                            </div>
                                        </td>
                                        <th scope="row"
                                        ...
                                        <td class="px-6 py-4">
                                            {{ $user->course }} {{ $user->section }}{{ $user->section1 }}
                                            {{ $user->time_preference }}
                                        </td>
                                        <td class="px-6 py-4">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-3">
                        <div class="w-full md:w-1/2 px-3">
                            ...
                            <input type="text" name="student_id" id="student_id"
                                ...
                                value="{{ old('student_id', $editUser->student_id ?? '') }}" required>
    ...
    <div id="promoteModal" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title"
        ...
        <div class="flex items-center justify-center min-h-screen">
            ...
            <div
                ...
                <form id="promoteForm" method="POST" action="{{ route('users.promote') }}">
                    @csrf
                    @method('PUT')
        ...
        function openModal(userId) {
            fetch(`/admin/users/${userId}/edit`)
                ...
                .then(data => {
                    ...
                    document.getElementById('profile_image').src = `{{ asset('storage/') }}/${data.profile_image}`;
                    ...
                    <div class="flex items-center mb-4">
                        <div class="w-24 h-24 overflow-hidden rounded-full mr-4">
                            <img class="w-full h-full object-cover"
                                src="{{ asset('storage/profile_images/default.jpg  ') }}" id="profile_image"
                                alt="">
                        </div>
                        <div class="flex-1">
                            <label
